    ...
    <section className="relative py-10 md:py-16 bg-s3-surface overflow-hidden section-grid noise-overlay">
      ...
      <div className="relative max-w-[1200px] mx-auto px-6">
        ...
        <motion.div
          ...
          className="max-w-2xl mx-auto mb-6 md:mb-9 rounded-xl p-4 md:p-5 flex items-start gap-3.5"
        ...
        {/* 業務理解→...→改善サポート フロー */}
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-stretch md:items-center gap-2.5 md:gap-2">
          {flow.map((f, i) => {
            const Icon = f.icon;
            const isLast = i === flow.length - 1;
            return (
              <div key={f.label} className="flex flex-col md:flex-row items-center gap-2.5 md:gap-2 md:flex-1">
                <motion.div
                  initial={{ opacity: 0, scale: 0.9 }}
                  whileInView={{ opacity: 1, scale: 1 }}
                  viewport={{ once: true, margin: "-30px" }}
                  transition={{ duration: 0.45, delay: i * 0.1 }}
                  className="card-luxury rounded-xl p-3.5 md:p-4 flex flex-row md:flex-col items-center gap-3 md:gap-2 w-full md:text-center"
                >
