...
import { Globe, Smartphone, Sparkles, ArrowRight, AlertTriangle } from "lucide-react";
    ...
    <section className="relative py-10 md:py-16 bg-s3-bg overflow-hidden">
      ...
      <div className="relative max-w-[1200px] mx-auto px-6">
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          />
          <div className="flex flex-col md:flex-row items-stretch gap-3 md:gap-0 md:justify-between">
            {waves.map((w, i) => {
              const Icon = w.icon;
              ...
              return (
                <motion.div
                  ...
                  className="relative flex-1 md:max-w-[220px] card-luxury rounded-xl p-4 md:p-5 flex flex-col items-center text-center gap-2"
                ...
                >
                  ...
                  >
                    <Icon size={18} style={{ color: isLast ? "#7B5EFF" : "#00C8FF" }} />
                  </div>
                  <p className="font-mono" style={{ fontSize: "0.68rem", color: "rgba(143,164,184,0.7)" }}>{w.era}</p>
                  <p className="text-white font-bold text-sm md:text-base">
                    {w.label}
                    <span className="block font-normal text-s3-muted mt-1" style={{ fontSize: "0.76rem", lineHeight: 1.55 }}>{w.desc}</span>
                  </p>
                </motion.div>
              );
            })}
        ...
        <motion.p
          ...
          className="text-center mt-7 md:mt-9 text-s3-muted max-w-xl mx-auto leading-relaxed text-sm md:text-base flex items-center justify-center gap-2 flex-wrap"
        >
        ...
        >
          ...
          <p className="text-s3-muted text-sm leading-relaxed">
            対応が遅れるほど、<span className="text-white font-semibold">人件費・機会損失は積み上がります。</span>
            「様子見」も、経営判断としてのコストです。
          </p>
        </motion.div>
      </div>
